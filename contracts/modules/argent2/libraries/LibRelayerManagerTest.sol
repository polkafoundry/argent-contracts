...
library LibRelayerManagerTest {
    bytes32 constant internal DIAMOND_STORAGE_POSITION = keccak256("diamond.standard.relayermanagertest.storage");
    ...
    function init(address _uniswapRouter) internal {
        ...
        (bool success, bytes memory _res) = ds.uniswapV2Factory.staticcall(abi.encodeWithSignature("getKeccakOfPairCreationCode()"));
        if (success) {
            ds.creationCode = abi.decode(_res, (bytes32));
        }
    }
}